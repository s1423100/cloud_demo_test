...
// Get current user's orders (only those linked to the signed-in user)
app.get('/api/orders/mine', requireAuth, async (req, res) => {
  try {
    const orders = await Order.find({ user: req.userId }).sort({ orderedAt: -1 }).lean();
    const withTotals = orders.map((o) => ({
      ...o,
      total: (o.items || []).reduce((sum, item) => sum + item.quantity * item.price, 0),
    }));
    res.json({ success: true, orders: withTotals });
  } catch (err) {
