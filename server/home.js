...
function deriveCategories(foods) {
  ...
  foods.forEach((f) => {
    ...
    map[key] = map[key] || {
      name: key === 'all' ? 'All items' : key.charAt(0).toUpperCase() + key.slice(1),
    };
  });
...
function bindFooterActions() {
  ...
  updateAuthButton();
  document.querySelector('.footer-buttons button:first-child').addEventListener('click', () => {
    openHistory();
  });
  document.querySelector('.footer-buttons button:last-child').addEventListener('click', () => {
    toggleCart(true);
  });
}
...
async function openHistory() {
  ...
  list.innerHTML = '';
  if (!token) {
    notice.textContent = 'Please sign in to view orders.';
    notice.className = 'notice error';
    toggleHistory(true);
    setTimeout(() => (window.location.href = 'login.html'), 800);
    return;
  ...
  try {
    ...
    if (!orders.length) {
      list.innerHTML = '<div class="history-row">No past orders.</div>';
    } else {
      orders.forEach((o) => {
        const row = document.createElement('div');
        row.className = 'history-row';
        const date = o.orderedAt ? new Date(o.orderedAt).toLocaleString() : '';
        const total = (o.total || 0).toFixed(2);
        const count = (o.items || []).reduce((sum, i) => sum + (i.quantity || 0), 0);
        row.innerHTML = `
          <div class="cart-name">Code: ${o.code || o._id}</div>
          <div class="meta">${count} item(s) • $${total} • ${date}</div>
        `;
    ...
    notice.textContent = err.message || 'Unable to load orders';
    notice.className = 'notice error';
    toggleHistory(true);
  }
}
